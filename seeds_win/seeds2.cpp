...
SEEDS::SEEDS(int width, int height, int nr_channels, int nr_bins, int min_size)
{
  this->width = width;
  this->height = height;
...
void SEEDS::initialize(UINT* image, int seeds_w, int seeds_h, int nr_levels)
{
  ...
  // convert input image to LAB or HSV
...
void SEEDS::assign_labels()
{
  ...
  int level = 0;
  int nr_seeds_w = cvFloor(width/seeds_w);
  int nr_seeds_h = cvFloor(height/seeds_h);
  int step_w = seeds_w;
  int step_h = seeds_h;
  ...
  for (int level = 1; level < seeds_nr_levels; level++)
  {
    ...
    for (int y=0; y<height; y++)
    {
      for (int x=0; x<width; x++)
      {
        ...
        if (label_y >= nr_seeds_h) label_y = nr_seeds_h-1;
        labels[level][y*width+x] = label_y*nr_seeds_w + label_x;
        parent[level-1][labels[level-1][y*width+x]] = labels[level][y*width+x]; // set parent
      }
    }
  }
...
void SEEDS::compute_histograms(int until_level)
{
  ...
  // initialize histograms in the first iteration
  if (iteration == 0)
  {
...
void SEEDS::update_blocks(int level, float req_confidence)
{
  ...
  for (int y=1; y<nr_h[level]-1; y++)  // Initial code is: for (int y=0; y<nr_h[level]; y++)  //ansh modified
  {
    for (int x=0; x<nr_w[level]-1; x++)
    {
  ...
  //initial code: for (int x=0; x<nr_w[level]; x++)
  for (int x=0; x<nr_w[level]-1; x++)
  {
    //initial code: for (int y=0; y<nr_h[level]-1; y++)
...
void SEEDS::update_pixels_means()
{
  ...
  if (forwardbackward)
  {
    ...
    for (int y=1; y<height-1; y++)
      ...
      for (int x=1; x<width-1; x++)
        for (int y=1; y<height-2; y++)  //initial code is: (int y=1; y<height-1; y++)  //ansh modified
        {
          int a11 = labels[seeds_top_level][(y-1)*width+(x-1)];
          int a12 = labels[seeds_top_level][(y-1)*width+(x)];
          int a13 = labels[seeds_top_level][(y-1)*width+(x+1)];
  ...
  else
  {
    ...
    for (int y=1; y<height-1; y++)
      ...
      for (int x=1; x<width-1; x++)
        for (int y=1; y<height-2; y++)  //initial code is: for (int y=1; y<height-1; y++)  //ansh modified
        {
          int a11 = labels[seeds_top_level][(y-1)*width+(x-1)];
          int a12 = labels[seeds_top_level][(y-1)*width+(x)];
          int a13 = labels[seeds_top_level][(y-1)*width+(x+1)];
...
void SEEDS::LAB2RGB(float L, float a, float b, int* R, int* G, int* B)
{
  ...
  fY = YT * pow((float)fY, (float)(1.0/3.0)) + (!YT) * (7.787 * fY + 16.0/116.0);

  float fX = a / 500.0 + fY;
  bool XT = fX > T2;
  float X = (XT * pow((float)fX, (float)3.0) + (!XT) * ((fX - 16.0/116.0) / 7.787));

  float fZ = fY - b / 200.0;
  bool ZT = fZ > T2;
  float Z = (ZT * pow((float)fZ, (float)3.0) + (!ZT) * ((fZ - 16.0/116.0) / 7.787));
...
int SEEDS::RGB2LAB(const int& r, const int& g, const int& b, float* lval, float* aval, float* bval)
{
  ...
  fX = XT * pow((float)xVal,(float)(1.0/3.0)) + (!XT) * (7.787 * xVal + 16.0/116.0);

  // Compute L
  float Y3 = pow((float)yVal,(float)(1.0/3.0)); 
  fY = YT*Y3 + (!YT)*(7.787*yVal + 16.0/116.0);
  lVal  = YT * (116 * Y3 - 16.0) + (!YT)*(903.3*yVal);

  fZ = ZT*pow((float)zVal,(float)(1.0/3.0)) + (!ZT)*(7.787*zVal + 16.0/116.0);
...
int SEEDS::RGB2LAB_special(int r, int g, int b, float* lval, float* aval, float* bval)
{
  ...
  fX = XT * pow((float)xVal,(float)(1.0/3.0)) + (!XT) * (7.787 * xVal + 16/116);
  ...
  fZ = ZT*pow((float)zVal,(float)(1.0/3.0)) + (!ZT)*(7.787*zVal + 16/116);
...
int SEEDS::count_superpixels()
{
  ...
  for (unsigned int i=0; i<nr_labels[seeds_top_level]; i++)
    count_labels[i] = 0;
  for (int i=0; i<width*height; i++)
